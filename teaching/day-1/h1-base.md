...
學員由 0 開始：開 Codex Desktop → 揀位放 project → clone 老師 skeleton repo → npm install → sync vault → dev server → 瀏覽器開 localhost:5173 見到 4 view render sample data。**今 hour 唔深入拆 codebase**，先 ship 跑得起再講。
...
- Sidebar 4 個 button：Today / Tasks / Daily Note / Vault Health
- Default Today view render 樣板 plan (3 件 task)
- 切去其他 view 都見到 sample data

**Wow moment #1**：4 個 view 已 work，我哋未寫過一行 code。
...
- [ ] localhost:5173 (dev) + localhost:4173 (preview) 都 render 到 4 view 嘅 sample data
